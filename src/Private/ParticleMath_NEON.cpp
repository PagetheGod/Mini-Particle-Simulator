...
namespace ParticleMath
{
    ...
    template void UpdateParticlePositionForAxis_Scalar<SIMDLevel::NEON>(float* restrict StartParticlePtr, uint32_t Count, const float* restrict Velocity, float DeltaTime);
    // Custom SIMD kernels — these dispatch through SIMDTraits<NEON> intrinsics
    template void SolvePointForce_Vector<SIMDLevel::NEON>(uint32_t StartParticleIndex, uint32_t Count, const glm::vec3& ForcePosition, const float Strength,
        const float Radius, const float DeltaTime, ParticleStates& ParticleStateRef);
    template void SolveVortex_Vector<SIMDLevel::NEON>(uint32_t StartParticleIndex, uint32_t Count, const float VortexStrength,
