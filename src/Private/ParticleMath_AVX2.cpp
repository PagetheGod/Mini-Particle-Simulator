...
namespace ParticleMath
{
    ...
    template void UpdateParticlePositionForAxis<SIMDLevel::AVX2>(float* restrict StartParticlePtr, uint32_t Count, const float* restrict Velocity, float DeltaTime);
    // Custom SIMD kernels — these genuinely dispatch through SIMDTraits<AVX2> intrinsics
    template void SolvePointForce<SIMDLevel::AVX2>(uint32_t StartParticleIndex, uint32_t Count, const glm::vec3& ForcePosition, const float Strength,
        const float Radius, const float DeltaTime, ParticleStates& ParticleStateRef);
    template void SolveVortex<SIMDLevel::AVX2>(uint32_t StartParticleIndex, uint32_t Count, const float VortexStrength,
